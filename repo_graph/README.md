...
`tools/code_graph/` generates deterministic, machine-readable code structure artifacts for AI agents.

The CLI is intentionally thin:

```bash
uv run python tools/code_graph/generate.py
uv run python tools/code_graph/generate.py --check
uv run python tools/code_graph/generate.py \
...
```bash
uv run python tools/code_graph/query_cli.py impacted server/routers/research_analysis.py
uv run python tools/code_graph/query_cli.py tests-for server/routers/research_analysis.py
uv run python tools/code_graph/query_cli.py endpoints-for-router server/routers/research_analysis.py
uv run python tools/code_graph/query_cli.py routes-for-view client/src/views/admin/ResearchAnalysisManager.vue
uv run python tools/code_graph/query_cli.py api-callers-for-endpoint /api/typing/sessions
uv run python tools/code_graph/query_cli.py e2e-for-route /typing
uv run python tools/code_graph/query_cli.py e2e-for-view client/src/views/TypingView.vue
uv run python tools/code_graph/query_cli.py --format json explain server/routers/research_analysis.py
```

The same commands are available through Make when shell quoting is more convenient:

```bash
make code-graph-query ARGS="tests-for server/routers/research_analysis.py"
```
...
```bash
make code-graph-query ARGS="api-callers-for-endpoint /api/typing/sessions"
make code-graph-query ARGS="e2e-for-route /typing"
make code-graph-query ARGS="e2e-for-view client/src/views/TypingView.vue"
```
...
```bash
CODE_GRAPH_USAGE_LOG=1 make code-graph-query ARGS="e2e-for-view client/src/views/TypingView.vue"
...
```bash
make code-graph-feedback ARGS="--task 'View change' --query 'e2e-for-view: client/src/views/TypingView.vue' --usefulness high"
make code-graph-feedback ARGS="missed --relation calls_api_endpoint --reason template_with_buildQuery --expected-source client/src/composables/useRecordingApi.js"
```
...
```bash
make code-graph-mcp
uv run python tools/code_graph/mcp_server.py --graph analysis/code_graph/repo_graph.json
```
...
Phase 6 adds an optional local pre-commit hook installer:

```bash
make code-graph-install-hook
```

The installer copies `tools/code_graph/hooks/pre-commit-code-graph` to `.git/hooks/pre-commit` only when no existing local pre-commit hook is present. It refuses to overwrite existing hooks. The hook is intentionally thin: it delegates to `make code-graph-check` and lets the Python entrypoints resolve `codegraph.config.toml`. It does not regenerate artifacts, stage files, or detect partial staging. CI remains the authoritative freshness gate. To avoid partial-staging drift, stage related source changes and `analysis/code_graph/` artifacts together. Watch mode and mandatory hook installation are intentionally deferred.
